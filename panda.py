...
class Panda(Ui_MainWindow):
	...
	def __init__(self, window, vSl=1024, parent=None):  # BUTOES
	...
	def encodeAPP(self):  # BUTAO ENCODE EXECUTAR FUNCAO
		...
		if check:
			...
			entrada = open(file, 'rb')
			_in = entrada.read()
			cod_ = struct.pack('II251s', ENCODED_OFFSET_SIZE, ENCODED_LEN_SIZE, str([_in]).encode())
